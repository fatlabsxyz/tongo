...
export function verifySameEncryptionSameKey(
  ...
  let res = verifyElGammal(L1, R1,g, y, AL1, AR1,c, sb,sr);
  if (res == false) {
    throw new Error("W1");
  }

  if (R1.equals(R2)) {return L1.equals(L2)};
  let Y = L2.subtract(L1);
  let G = R2.subtract(R1);
  res = poe(Y, G, A, c, s);
  if (res == false) {
    throw new Error("Q1");
  }
...
/// EC_MUL: 10
/// EC_ADD: 6
export function verifySameEncryption(
  L1: ProjectivePoint,
  R1: ProjectivePoint,
  L2: ProjectivePoint,
  R2: ProjectivePoint,
...
//TODO: Refactor this to handle te correct proof. (Ask to Alba)
