...
import { codec, externalData, ExternalData, relayData, RelayData, TransferOptions } from "./fixtures";

const transferOptionsType = "core::option::Option::<tongo::structs::operations::transfer::TransferOptions>" as const;
...
describe('TransferOptions encode/decode round-trip', () => {
  ...
  it("Some(relayData: Some, externalData: Some)", () => {
    const encoded = codec.encode(transferOptionsType, transferOptionsSomeSomeRelayDataSomeExternalData);
    const decoded = codec.decode(transferOptionsType, encoded);
    expect(decoded).toEqual(transferOptionsSomeSomeRelayDataSomeExternalData);
  });

  it("Some(relayData: None, externalData: Some)", () => {
    const encoded = codec.encode(transferOptionsType, transferOptionsSomeNoneRelayDataSomeExternalData);
    const decoded = codec.decode(transferOptionsType, encoded);
    expect(decoded).toEqual(transferOptionsSomeNoneRelayDataSomeExternalData);
  });

  it("Some(relayData: Some, externalData: None)", () => {
    const encoded = codec.encode(transferOptionsType, transferOptionsSomeSomeRelayDataNoneExternalData);
    const decoded = codec.decode(transferOptionsType, encoded);
    expect(decoded).toEqual(transferOptionsSomeSomeRelayDataNoneExternalData);
  });

  it("Some(relayData: None, externalData: None)", () => {
    const encoded = codec.encode(transferOptionsType, transferOptionsSomeNoneRelayDataNoneExternalData);
    const decoded = codec.decode(transferOptionsType, encoded);
