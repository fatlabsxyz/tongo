...
function bumpResourceBounds(
    rb: EstimateFeeResponseOverhead["resourceBounds"],
): EstimateFeeResponseOverhead["resourceBounds"] {
    const { l2_gas, l1_data_gas, l1_gas } = rb;
    return {
        l1_data_gas,
        l1_gas,
        l2_gas: {
...
function getCasmSierra(manifestDir: string, manifest: ContractManifest) {
    return {
        contract: JSON.parse(
            fs.readFileSync(path.join(manifestDir, manifest.artifacts.sierra), ENC_UTF8),
        ),
        casm: JSON.parse(
            fs.readFileSync(path.join(manifestDir, manifest.artifacts.casm), ENC_UTF8),
        ),
    };
}
...
export async function setupContracts() {
    ...
    const manifest: { contracts: ContractManifest[] } = JSON.parse(
        fs.readFileSync(manifestPath, ENC_UTF8),
    );
    ...
    const TONGO_DEPLOYED_EVENT_KEY =
        "0x221ac4e024d25df3a671a87e4648c6575290d2a0be45f1ff096a99c5be5eef7";
    const { events } = await Vault.invoke("deploy_tongo", args.calldata! as string[], {
        waitForTransaction: true,
    });
    const { data, keys } = events.find(
        ({ from_address, keys: [firstKey] }) =>
            from_address === VaultAddress && firstKey === TONGO_DEPLOYED_EVENT_KEY,
    )!;
    const tongoDeployedEvent = vaultCodec.decodeEvent("tongo::structs::events::TongoDeployed", {
        keys,
        data,
    });
