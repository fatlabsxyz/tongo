...
use crate::consts::{RELAYER_ADDRESS, STRK_ADDRESS, USER_ADDRESS, VAULT_ADDRESS};
use crate::prover::utils::{decipher_balance, generate_random, pubkey_from_secret};
use crate::tongo::operations::{
    fundOperation, ragequitOperation, transferOperation, withdrawOperation,
};
...
fn test_tongo_relay_ragequit() {
    ...
    let (operation, ragequit_options) = ragequitOperation(
        x, initial_fund, transfer_address, sender, fee_to_sender, dispatcher,
    );
...
fn test_tongo_relay_withdraw() {
    ...
    assert(finalErc20Relayer - initialErc20Relayer == rate * (fee_to_sender).into(), 'nope');
    assert!(
        initialErc20Vault - finalErc20Vault == rate * (withdraw_amount + fee_to_sender).into(),
        "Incorrect balance for Vault",
    );
}
...
fn test_tongo_relay_transfer() {
    ...
    let (operation, transfer_options) = transferOperation(
        x, y_bar, transfer_amount, initial_fund, sender, fee_to_sender, tongo_address, dispatcher,
    );
    start_cheat_caller_address(tongo_address, sender);
