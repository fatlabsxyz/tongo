...
/// Balances are encrypted with ElGammal, which consists in a tuple of curve points (L, R).
/// Internally the points are constructed with L = g**b y**r, R = g**r where g is the generator of
/// the starknet curve, y is a pubkey, r is a random value and b is the balance to encrypt.
...
pub trait CipherBalanceTrait<CipherBalance> {
    ...
    /// Returns a new CipherBalance which componentes are the product of the components of the two
    /// given CipherBalances.
    /// Homomorphically, if the two given CipherBalances where encrypted under the same PubKey, the
    /// result is a valid CipherBalance of the sum of the amounts ciphered the inputs, under the
    /// same PubKey.
    fn add(self: CipherBalance, cipher: CipherBalance) -> CipherBalance;

    /// Returns a new CipherBalance which componentes are the divistion of the components of the two
    /// given CipherBalances.
    /// Homomorphically, if the two given CipherBalances where encrypted under the same PubKey, the
    /// result is a valid CipherBalance of the difference of the amounts ciphered in each of the
    /// inputs, under the same PubKey.
    fn subtract(self: CipherBalance, cipher: CipherBalance) -> CipherBalance;

    /// Returns a CipherBalance with components {L: {x:0, y:0}, R:{x:0, y:0}}. This is not a valid
    /// CipherBalance because {x:0, y:0} is not a curve point of the starket curve. This is only
    /// usefull because the Default value for a CipherBalance in the storage of the contract is this
    /// null and represents a stored balance of 0.
    fn null() -> CipherBalance;

    /// Check if the given CipherBalance is null.
    fn is_null(self: @CipherBalance) -> bool;

    /// If the given CipherBalance is null, returns a valid CipherBalance for the pubkey that stored
    /// the value 0.
