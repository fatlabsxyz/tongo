...
/// because the members x, y of PubKey are (in theory) not constrained to be the x and y coordinates of a
/// curve point.
...
/// Implements Serde for PubKey. Serde is used by the protocol when deserializes the calldata (Span<felt252>) of a tx.
/// By implementing this on our own,  the protocol will throw an error if a PubKey is not well formed, i.e. x and y are not
/// the coordinates of a curve point.
